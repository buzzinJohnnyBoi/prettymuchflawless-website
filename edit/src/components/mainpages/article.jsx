...
class article extends Component {
    ...
    sendData = async(text, image, video) => {
        ...
        const path = window.location.pathname.replace('article/', '');
        const articlePath = path.replace('/', '');  
        let res = await Axios.post('http://192.168.1.240:3003/updateArticle', {layout: sendData, link: articlePath});
    ...
    async getPage() {
        try {
            ...
            document.title = ((articlePath == '') ? 'Home' : articlePath) + ' - Articles - Pretty Much Flawless';
